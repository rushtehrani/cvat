...
import {
    Button, Col, Input, Modal, notification, Row, Select, Spin,
} from 'antd';
...
export default class ModelNewAnnotationModalComponent extends React.PureComponent<Props, State> {
    private static ExecuteSuccessMessage = (name: string, url: string): JSX.Element => (
        <div>
            {name}
            {' '}
            workflow has been executed. Please check the workflow for logs.
            <br />
     ...
     * @param value
     */
    onWorkflowParamSelectChange = async (workflowParams: any, value: string): Promise<any> => {
        const { onSysRefModelUpdated: onSysRefModelUpdated1, setState, state } = this;
        setState({
            selectedWorkflowParam: {
                ...state.selectedWorkflowParam,
                [workflowParams.name]: value,
            },
        });

        if (workflowParams.name === 'cvat-model') {
            await onSysRefModelUpdated1(workflowParams, value);
        }
    };
     ...
     * @param value
     */
    onSysRefModelUpdated = async (workflowParam: any, value: string): Promise<any> => {
        const { updateSysFinetuneCheckpoint, state } = this;
        if (!state.workflowTemplate) {
            return;
        }

        const sysFinetuneCheckpoint = state.allSysFinetuneCheckpoint;
        const workflowTemplateUid = state.workflowTemplate.uid;
    ...
    private async onExecuteWorkflow(): Promise<void> {
        ...
        let dumpFormat;
        if (selectedDumpFormat === undefined) {
            dumpFormat = { tag: '', name: '' };
        } else {
            dumpFormat = selectedDumpFormat;
        }
        const finalPayload: ExecuteWorkflowPayload = {
            workflow_template: workflowTemplate.uid ? workflowTemplate.uid : '',
            parameters: selectedWorkflowParam,
            dump_format: dumpFormat.tag,
        };
        if (selectedNodePool) {
        ...
        if ((selectedFinetuneCheckpoint || allSysFinetuneCheckpoint.value) && selectedFinetuneCheckpoint !== 'none') {
            finalPayload.parameters['cvat-finetune-checkpoint'] = selectedFinetuneCheckpoint || allSysFinetuneCheckpoint.value;
        }

        try {
            const baseUrl: string = core.config.backendAPI.slice(0, -7);
            const successResp = await core.server.request(`${baseUrl}/onepanelio/execute_workflow/${taskInstance.id}`, {
                method: 'POST',
                data: finalPayload,
                headers: {
            ...
            notification.open({
                ...
                description: ModelNewAnnotationModalComponent.ExecuteSuccessMessage(
                    workflowTemplate.uid, successResp.url,
                ),
            });
     ...
     * @private
     */
    private updateSysFinetuneCheckpoint =
    async (sysFinetuneCheckpoint: any, workflowTemplateUid: string, sysRefModel?: string): Promise<any> => {
        this.setState({
            updatingModel: true,
        });
        ...
        this.setState({
            allSysFinetuneCheckpoint: {
                ...
                display_name:
                    sysFinetuneCheckpoint.display_name ?
                        sysFinetuneCheckpoint.display_name : sysFinetuneCheckpoint.name,
            },
            updatingModel: false,
        });
    };
        ...
        try {
            ...
            if (sysNodePoolParam) {
                ...
                this.setState({
                    ...
                    defaultSysNodePoolVal: nodePool.options[0].value,
                    selectedNodePool: nodePool.options[0],
                });
            } else {
                this.setState({
            ...
            if (sysAnnotationPath) {
                ...
                this.setState({
                    sysAnnotationPath: {
                        display_name: sysAnnotationPath.display_name ?
                            sysAnnotationPath.display_name : sysAnnotationPath.name,
                        ...
                        <Row type='flex' align='middle' key={index}>
                            <Col span={24}>
                                <label className='cvat-text-color ant-form-item-label'>
                                    { workflowParams.display_name ? workflowParams.display_name : workflowParams.name }
                                    :
                                </label>
                                {
...

                {
                    this.state.allSysFinetuneCheckpoint.options !== null ? (
                        <Row type='flex' align='middle'>
                            <Col span={24}>
                                <label className='cvat-text-color ant-form-item-label'>
                                    {this.state.allSysFinetuneCheckpoint.display_name}
                                    :
                                </label>
                                <Select
...

                {
                    this.state.sysOutputPath.value ? (
                        <Row type='flex' align='middle'>
                            <Col span={24}>
                                <label className='cvat-text-color ant-form-item-label'>
                                    {this.state.sysOutputPath.display_name}
                                    :
                                </label>
                                <TextArea
...

                {
                    this.state.sysAnnotationPath.value ? (
                        <Row type='flex' align='middle'>
                            <Col span={24}>
                                <label className='cvat-text-color ant-form-item-label'>
                                    {this.state.sysAnnotationPath.display_name}
                                    :
                                </label>
                                <TextArea
                                    autoSize={{ minRows: 1, maxRows: 4 }}
